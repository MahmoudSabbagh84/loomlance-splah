...
4. ✅ **Sign-in + signup done.** **Signin / F2 + remember-me + F5:** splash `signin.html` → hostname-aware redirect to dashboard `/login`; dashboard `LoginPage` got Remember-me (`lib/authStorage.js` switchable storage adapter, 9 tests); F5 was a no-op. **Signup / F3 (done):** rebuilt `signup.html` on the system — lean fields (Full name → `display_name`, email, password+reqs, confirm, terms, marketing, plan selector w/ `?plan=`); dropped username + phone + the fake card/payment section; metadata `{ display_name, selected_plan, plan_name, marketing_emails }`. Playwright-verified (plan select, URL param, validation, password toggle, 0 console errors). **`styles.css` deleted** (signup + signin were its last consumers).
5. ✅ **Design-system doc rewritten (done):** `LoomLance-Design-System.md` is now **v2.0 "Slate Pro"** — documents the violet/Tailwind system (tokens light+dark, Tailwind mapping, Outfit type scale, the real `input.css` component classes, layout/band patterns, build/tooling, clean-URL convention), explicitly supersedes the retired orange v1.0. Values cross-checked against `src/tokens.css` / `tailwind.config.js` / `src/input.css`.

> **Resume:** ✅ **Splash redesign COMPLETE.** All pages on the violet system, `styles.css` deleted, design-system doc current. Nothing outstanding here. (Site-wide go-live chores — Stripe live cutover, AWS root-key cleanup, Supabase Auth URL config — are tracked in the dashboard `context.md`, not this spec.)
